...
def test(model, test_gen, criterion, device):
    ...
    # Used for storing all error metric values, to calculated SD later
    MSE_05_ALL = []
    MSE_10_ALL = []
    MSE_15_ALL = []
    FMSE_ALL = []
    ...
    CMSE_ALL = []
    CFMSE_ALL = []
    allData = {'ADE_05': [], 'ADE_10': [], 'ADE_15': [], 'CADE': [], 'CFADE': []}
    model.eval()
    ...
    with torch.set_grad_enabled(False):
        for batch_idx, data in enumerate(loader):
            ...
            batch_MSE_15, batch_MSE_05, batch_MSE_10, batch_FMSE, batch_CMSE, batch_CFMSE, batch_FIOU, MSE_total_dict= \
            ...
            for error_metric, datas in MSE_total_dict.items():
                for x in datas:
                    allData[error_metric].append(x)
            # MSE_05_ALL.append(batch_MSE_05)
            # MSE_10_ALL.append(batch_MSE_10)
            # MSE_15_ALL.append(batch_MSE_15)
            # FMSE_ALL.append(batch_FMSE)
            # CMSE_ALL.append(batch_CMSE)
            # CFMSE_ALL.append(batch_CFMSE)
            # FIOU_ALL.append(batch_FIOU)
    ...
    SD = {
        'ADE_05': np.std(allData['ADE_05']),
        'ADE_10': np.std(allData['ADE_10']),
        'ADE_15': np.std(allData['ADE_15']),
        'CADE': np.std(allData['CADE']),
        'CFADE': np.std(allData['CFADE'])
    }
    ...
    # test_loss = total_goal_loss / count + total_dec_loss / count # ETH det
    # test_loss = total_goal_loss / count + total_cvae_loss / count + total_KLD_loss / count # ETH cvae
    # return test_loss, MSE_15, MSE_05, MSE_10, FMSE, FIOU, CMSE, CFMSE, SD, allData
    return MEAN, SD, allData
